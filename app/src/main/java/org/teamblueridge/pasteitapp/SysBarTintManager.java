...
public class SysBarTintManager {
    ...
    /**
     * Checks the system to see if translucent decor is enabled.</p>
     * <p/>
     * If translucency is enabled, the activity window applies the specified flags.</p>
     * <p/>
     * This should be called before super.onCreate(bundle)</p>
     *
     * @param activity             The {@link Activity} to apply the {@link WindowManager.LayoutParams} flags
     * @param translucentStatusBar {@code true} to set the {@link WindowManager.LayoutParams#FLAG_TRANSLUCENT_STATUS}
     *                             flag
     * @param translucentNavBar    {@code true} to set the
     *                             {@link WindowManager.LayoutParams#FLAG_TRANSLUCENT_NAVIGATION} flag
     * @return {@code true} if the flags were set, {@code false} otherwise.
     */
    @TargetApi(Build.VERSION_CODES.KITKAT)
    public static boolean setupTranslucency(Activity activity, boolean translucentStatusBar,
                                            boolean translucentNavBar) {
        int id = activity.getResources().getIdentifier("config_enableTranslucentDecor", "bool",
                "android");
        if (id != 0) {
            try {
                ...
                android.util.Log.w(SysBarTintManager.class.getSimpleName(),
                        "Error setting system window flags.", t);
            }
        }
        return false;
    }
     ...
     *
     * @param enabled {@code true} to enable coloring the {@link ActionBar}
     */
